    ...
    <>
            ...
            <div className="answer-section">
              ...
              {query[currentQuestion].answerOption.map((answerOption,options,index) => (
                ...
                <ul className='unList'   onClick={()=>{ setPrime(answerOption.id);setStyle(options) ;scoreCard(answerOption.iscorrect)}}>  
                  <li className='list' 
                    key={index}  
                    style={{ 
                    backgroundColor: style === options ? '#5e69ff' : '',
                    }} > 
                    {answerOption.answer} 
            ...
            <button  className='start-button'  onClick={handleAnswerButtonClick} > {currentQuestion+1 === 5
            ? "Submit":"Next"}</button>
